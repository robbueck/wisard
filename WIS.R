...
run_everything <- function(in_file, len_filter=0, e_filter=10,
  ...
  if (length(blast_data$alignments) < 1) {
    ...
    mean_overlap <- mean(countOverlaps(blast_data$alignments, blast_data$alignments, ignore.strand = T))
    logr <- log(length(blast_data$alignments)/mean_overlap)
    logr2 <- log(sum(
      width(
        disjoin(blast_data$alignments)
        )
      )/mean(width(blast_data$alignments)))
    logr <- (logr + logr2)/2
    # blast_data$alignments$score <- (blast_data$alignments$raw_score *
    #                                   mdata$Statistics_lambda - 1 -
    #                                   log(mdata$Statistics_kappa *
    #                                         mdata$query_len *
    #                                         mdata$Statistics_db.len) + logr)
    #blast_data$alignments$score <- blast_data$alignments$raw_score
    blast_data$alignments$score <- blast_data$alignments$Hsp_align.len
    start_l <- Sys.time()
    ...
    out_list <- list(final = out_data,
                     ...
                     logr_exp=logr,
                     logr_calc=log(length(out_data$alignments)),
                     logr_exp_alt=logr2)
    return(out_list)
  }
}
...
blast_overhang <- 0
threads <- 3
outstats_name <- "bacteria_outstats_tblastx_max_len.txt"
...
blast_files <- list.files(path=dirname, pattern="*_ab_tblastx.xml", full.names=TRUE, recursive=FALSE)
sizes <- file.info(blast_files)$size
blast_files <-  blast_files[which(sizes != 0)]
print("number of files to analyze:")
print(length(blast_files))




outstats <- lapply(blast_files, function(x) {
  print(x)
  ...
  outfile <- gsub(".xml", "_max_len_wis.tab", x)
  ...
  stats <- data.table(genome=gname,
                      ...
                      total_raw=sum(final_alis$raw_score),
                      total_raw_greedy=sum(greedy_alis$raw_score),
                      time_wis=out$wis_time,
                      time_greedy=out$greedy_time,
                      memory_wis=out$memory_wis,
                      memory_greedy=out$memory_greedy,
                      logr_expected=out$logr_exp,
                      logr_calculated=out$logr_calc,
                      logr_expected_alt=out$logr_exp_alt
  )
